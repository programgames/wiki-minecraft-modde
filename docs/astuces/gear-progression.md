...
```mermaid
flowchart LR
    A[Vanilla Basics] --> B[Modded Systems] --> C[Advanced Gear] --> D[Ultimate Gear]

    subgraph early["EARLY GAME"]
        A
    end
...
```mermaid
flowchart TB
    subgraph phase1["PHASE 1: EARLY GAME (0-10h)"]
        E1[Iron Gear<br/>Basic] --> E2[Diamond Gear<br/>Enchants] --> E3[Tinker's Tools<br/>Modifiers] --> E4[Silent Gear<br/>Traits]
    end

    subgraph phase2["PHASE 2: MID GAME (10-30h)"]
        M1[Mekanism Tools<br/>Upgradeable] --> M2[Bound Tools<br/>Blood Magic] --> M3[Terrasteel Set<br/>Mana-Powered]
    end

    subgraph phase3["PHASE 3: LATE GAME (30-60h)"]
        L1[Draconic Gear<br/>Dragon Power] --> L2[MekaSuit<br/>Ultimate Tech] --> L3[Creative Items<br/>Endgame]
    end
...
```mermaid
flowchart LR
    S[Steel<br/>Base] --> B[Bronze<br/>Better] --> L[Lapis Lazuli<br/>Enhanced] --> O[Osmium<br/>Advanced]
    O --> RO[Refined Obsidian<br/>EXCELLENT] --> RG[Refined Glowstone<br/>Specialized]
```
...
```mermaid
flowchart LR
    I[Iron Armor<br/>Protection: 15<br/>Toughness: 0<br/>Facile] --> D[Diamond Armor<br/>Protection: 20<br/>Toughness: 8<br/>Intermédiaire] --> N[Netherite Armor<br/>Protection: 20<br/>Toughness: 12<br/>Knockback Res: 4<br/>Enchantable++]
```
...
```mermaid
flowchart TB
    W[Wyvern<br/>RF Shield, Flight<br/>Dragon core] --> C[CREATIVE TIER<br/>Endgame absolu]
    D[Draconic<br/>Shield+++, Flight+++<br/>Chaos Shard, OP stats] --> C
    M[MekaSuit<br/>Modular, Flight<br/>Tech-based, Upgradeable] --> C
```
...
```mermaid
flowchart LR
    subgraph p1["Phase 1 (ASAP)"]
        A1[Sojourner's Sash] --> B1[Mobilité de base]
    end
    subgraph p2["Phase 2 (Early-Mid)"]
        A2[Ring of Magnetism] --> B2[Farming efficace]
    end
    subgraph p3["Phase 3 (Mid)"]
        A3[Flight Ring/Item] --> B3[Exploration libre]
    end
    subgraph p4["Phase 4 (Mid-Late)"]
        A4[Ring of Odin] --> B4[Survie boss]
    end
    subgraph p5["Phase 5 (Late)"]
        A5[Ring of the Aesir] --> B5[Ultimate combo]
    end
...
```mermaid
flowchart TB
    Q1{Votre outil actuel<br/>vous satisfait?}
    Q1 -->|OUI| KEEP[KEEP]
    Q1 -->|NON| Q2{Pourquoi?}
    ...
    Speed --> Q3{Peut-on améliorer<br/>dans le système?}
    Durabilité --> Q3
    Effets --> Q3
    Puissance --> Q3

    Q3 -->|OUI| UPGRADE[UPGRADE]
    Q3 -->|NON| CHANGE[CHANGE SYSTEM]
```
